...
namespace C
{
    ...
    public static class ThreatResponseEngine
    {
        ...
        private static List<DrillQuestion> drillSet = new List<DrillQuestion>();
        ...
        public static string InitialiseDrill()
        {
            ...
            drillCorrect = 0;
            DrillActive = true;
            ActivityLogged?.Invoke($"Drill started with {drillSet.Count} questions");
        ...
        public static string EvaluateDrillAnswer(int selection)
        {
            ...
            if (drillCursor >= drillSet.Count)
            {
                ...
                ActivityLogged?.Invoke($"Drill complete: {drillCorrect}/{drillSet.Count}");
        ...
        private static string FormatDrillQuestion()
        {
            ...
            return $"📋 Question {drillCursor + 1} of {drillSet.Count}\n────────────────────────────────────────────────\n{q.Prompt}\n\n 1. {q.Choices[0]}\n 2. {q.Choices[1]}\n 3. {q.Choices[2]}\n 4. {q.Choices[3]}\n\nReply with 1, 2, 3, or 4.";
        }
        ...
        public static string Dispatch(string rawInput, SecurityBot bot, TaskStore? taskStore = null, LogBook? logger = null)
        {
            if (string.IsNullOrWhiteSpace(rawInput)) return string.Empty;

            string lower = rawInput.ToLowerInvariant().Trim();

            if (!string.IsNullOrEmpty(agentName) && logger != null)
            {
                logger.Log("User input received", rawInput);
            }
            ...
            if (DrillActive)
            {
                ...
                {
                    string result = EvaluateDrillAnswer(n);
                    if (logger != null && result.Contains("DRILL FINISHED"))
                    {
                        logger.Log("Drill completed", $"Score: {drillCorrect}/{drillSet.Count}");
                    }
            ...
            if (string.IsNullOrEmpty(agentName))
            {
                ...
                if (logger != null)
                {
                    logger.Log("Agent registered", $"Agent {agentName} joined");
                }
            ...
            {
                ActivityLogged?.Invoke("Drill started by user");
            ...
            {
                AgentMood.Alarmed => $"⚠️ Understood, Agent {agentName}. Stay calm — I'll guide you.\n\n",
                AgentMood.Irritated => $"Got it, Agent {agentName}. I'll keep it simple.\n\n",
                AgentMood.Inquisitive => $"Good question, Agent {agentName}. Here's the full breakdown.\n\n",
                _ => string.Empty
            };
            ...
            {
                repeatAlert = $"\n\n💡 You've opened '{lastModule}' {accessCount} times. Try a drill to test what you know! (type 'drill')";
            }

            if (lower.Contains("more") || lower.Contains("another") || lower.Contains("expand") || lower.Contains("elaborate"))
            {
                if (!string.IsNullOrEmpty(lastModule))
            ...
            if (lower.Contains("show task") || lower.Contains("list task") || lower.Contains("view task"))
            {
                if (taskStore != null)
                {
                    var tasks = taskStore.GetAllTasks();
                    if (!tasks.Any())
                        return LogAndReturn("📋 No tasks found. Add one with 'add task: [description]'");

                    string summary = "📋 YOUR SECURITY TASKS\n────────────────────────────────────────────────\n";
                    int i = 1;
                    foreach (var t in tasks.OrderByDescending(t => t.CreatedAt))
                    {
                        string status = t.IsComplete ? "✅" : "⬜";
                        string reminder = t.ReminderDate.HasValue ? $" (Reminder: {t.ReminderDate.Value:yyyy-MM-dd})" : "";
                        summary += $"{i++}. {status} {t.Title}{reminder}\n";
                    }
                    return LogAndReturn(summary);
                }
            }
                ...
                lower.Contains("vish") || lower.Contains("smish") || lower.Contains("spam"))
            {
                ...
                if (logger != null) logger.Log("Module accessed", "Phishing briefing");
                ...
                lower.Contains("two factor") || lower.Contains("vault"))
            {
                ...
                if (logger != null) logger.Log("Module accessed", "Password/MFA briefing");
                ...
                lower.Contains("url") || lower.Contains("link") || lower.Contains("internet"))
            {
                ...
                if (logger != null) logger.Log("Module accessed", "Secure browsing briefing");
                ...
                lower.Contains("data protection"))
            {
                ...
                if (logger != null) logger.Log("Module accessed", "Privacy briefing");
                ...
                lower.Contains("spyware") || lower.Contains("infected") || lower.Contains("antivirus"))
            {
                ...
                if (logger != null) logger.Log("Module accessed", "Malware briefing");
                ...
                lower.Contains("tailgat") || lower.Contains("manipulation") || lower.Contains("usb drive"))
            {
                ...
                if (logger != null) logger.Log("Module accessed", "Social engineering briefing");
                ...
                lower.Contains("compromised") || lower.Contains("account stolen") || lower.Contains("what do i do"))
            {
                ...
                if (logger != null) logger.Log("Module accessed", "Incident response protocol");
        ...
        {
            string agent = string.IsNullOrEmpty(agentName) ? "Agent" : agentName;
            return $"📋 SECURESHIELD HELP MENU — Agent {agent}\n\n" +
